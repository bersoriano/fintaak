...
export const components: PortableTextComponents = {
  block: {
    h2: ({ children }) => (
      <h2
        className="text-3xl font-bold text-[#2D3142] mt-12 mb-4"
    ...
    h3: ({ children }) => (
      <h3
        className="text-2xl font-semibold text-[#2D3142] mt-8 mb-3"
        style={{ fontFamily: 'var(--font-poppins)' }}
      >
        {children}
      </h3>
    ...
    h4: ({ children }) => (
      <h4
        className="text-xl font-semibold text-[#2D3142] mt-6 mb-2"
        style={{ fontFamily: 'var(--font-poppins)' }}
      >
        {children}
      </h4>
  ...
  marks: {
    ...
    code: ({ children }) => (
      <code className="bg-gray-100 text-[#2E7D32] px-2 py-1 rounded text-sm font-mono">
        {children}
      </code>
    ),
    link: ({ children, value }) => {
